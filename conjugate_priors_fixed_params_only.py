...
def metropolis_hastings_subtype_conjugate_priors(
    ...
    real_order1_dict = orders['1.0']['order']
    real_order2_dict = orders[f"{target_tau}"]['order']
    ...
    total_log_likelihood_history = []
    subtype_assignments_history = []
    total_tau_history = []
    subtype_accuracy_history = []

    acceptance_count1 = 0
    acceptance_count2 = 0
    
    for iteration in range(iterations):
        total_log_likelihood_history.append(total_ln_likelihood)
        ...
        total_tau_history.append(total_tau)
        subtype_accuracy_history.append(subtype_accuracy)
        ...
        # Likelihood of New Assignment + Old Order
        ln_likelihood1 = 0 
        ln_likelihood2 = 0
        ...
        # Calculate total tau
        _, _, _, _, tau1, tau2, paring_result = run.best_kendall_tau_pairing(
            real_order1_dict,
            real_order2_dict,
            order1_dict,
            order2_dict,
        ...
        subtype_accuracy = run.compute_subtype_accuracy(subtype_assignment, subtype_ground_truth, paring_result)

        # Calculate total ln_likelihood 
        total_ln_likelihood = ln_likelihood1 + ln_likelihood2
    ...
    return all_orders, log_likelihoods, subtype_assignments_history, total_tau_history, subtype_accuracy_history, total_log_likelihood_history